...
### Recommended Fixes
{fix_text or '_See Orbit Rover analysis on the merge request._'}

---
<sub>Created by [Orbit Rover]({settings.dashboard_base_url}/analyses/{analysis_id}) · Pipeline [#{record.pipeline_iid}]({record.pipeline_url or '#'})</sub>
"""

  gitlab = GitLabClient(settings)
  issue = await gitlab.create_issue(
    record.project_id,
    title=f"[Orbit Rover] Fix: {record.cause[:80]}",
    description=description,
    labels=["orbit-rover", "pipeline-failure", "bug"],
  )
